...
export const createExerciseInRoutine = async (
  exercise: Exercise,
  routineId: string
): Promise<Exercise> => {
  try {
    let response = await dbAxiosInstance.post(
      `/exercises/${routineId}`,
      exercise
    );
    return response.data;
...
export const addExerciseIdToRoutine = async (
  exercise: Exercise,
  routineId: string
) => {
  try {
    const response = await dbAxiosInstance.patch(`/routines/${routineId}`, {
      exercise,
    });
    return response.data;
  } catch (error) {
    console.error("Error updating routine with new exercise ID", error);
    throw error;
  }
};
